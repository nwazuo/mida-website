...
export const SEO = (props: MetaProps) => {
  ...
  const resolvedMetadata = React.useMemo(() => {
    const data = {
      title: props.title
        ? `${props.title} - ${props.defaultSiteTitle}`
        : props.defaultSiteTitle,
      description: props.description ?? props.defaultSiteDescription,
      canonical: props.cannonical ?? (isClient ? window.location.href : ''),
      ogImage: {
        url: props.ogImage ?? defaultMeta.ogImage,
        alt: props.title ?? props.defaultSiteTitle,
        width: 1200,
        height: 630,
      },
      twitter: {
